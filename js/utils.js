...
// ==================== TEMA (CORRIGIDO COM MERGE TRUE) ====================
let themeInterval = null;
function setTheme() {
    const sel = document.getElementById('themeSelect');
    if (!sel) return;
    const mode = sel.value;
    // Salva no Firestore se logado (usando set com merge para evitar erro se o documento não existir)
    if (usuarioLogado) {
        db.collection('vendedores').doc(`${usuarioLogado.unidade}_${usuarioLogado.numero}`).set({
            tema: mode
        }, { merge: true }).catch(() => { /* falha silenciosa */ });
    }
    applyTheme(mode);
}
