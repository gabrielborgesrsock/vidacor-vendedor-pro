...
function setTheme() {
    ...
    if (usuarioLogado) {
        db.collection('vendedores').doc(`${usuarioLogado.unidade}_${usuarioLogado.numero}`).set({
            tema: mode
        }, { merge: true }).catch(() => {});
    }
    applyTheme(mode);
...
// ==================== NAVEGAÇÃO (com chatScreen) ====================
const telas = ['dashboardScreen','listaScreen','cadastroScreen','detalheScreen','mapaScreen','ferramentasScreen','chatScreen'];
function mostrarTela(id) {
    ...
    if (id === 'dashboardScreen') {
        if (typeof carregarDashboard === 'function') carregarDashboard();
        if (typeof obterClima === 'function') obterClima();
    }
    ...
    if (id === 'chatScreen' && typeof iniciarChat === 'function') iniciarChat();
    window.scrollTo(0, 0);
}
