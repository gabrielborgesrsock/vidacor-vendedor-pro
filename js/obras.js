// obras.js - CRUD de obras, listagem com paginação + listener híbrido, subcoleções para fotos e histórico

const OBRAS_POR_PAGINA = 20;
let ultimoDocumento = null;   // controle da paginação
let obrasPagina = [];         // obras da página atual
let unsubscribeAtualizacoes = null; // listener para atualizações em tempo real

// ==================== FIRESTORE LISTENER (PAGINADO + HÍBRIDO) ====================
function iniciarListenerObras() {
    if (!usuarioLogado || !usuarioLogado.unidade) return;
    if (unsubscribeObras) unsubscribeObras();      // limpa listener antigo (se existir)
    carregarPagina(1);
}
...
async function carregarPagina(direcao) {
    ...
    try {
        ...
        // Mescla com array global (para compatibilidade com outras funções)
        obras = obrasPagina;
        atualizarTelaAtual();
        atualizarFiltrosVendedor();

        // Ativa listener híbrido para os documentos exibidos
...
function ativarListenerHibrido() {
    ...
    // Escuta apenas os documentos que estão na página atual
    unsubscribeAtualizacoes = db.collection('obras')
        ...
        .onSnapshot(snapshot => {
            ...
            console.error('Erro no listener híbrido:', error);
        });
}

// Função para carregar próxima página (botão "Carregar mais")
async function carregarMaisObras() {
    ...
    try {
        ...
        snapshot.forEach(doc => {
            obrasPagina.push({ ...doc.data(), firestoreId: doc.id });
        });
        ultimoDocumento = snapshot.docs[snapshot.docs.length - 1];
        obras = obrasPagina;
        listarObras();

        // Reativa listener híbrido com os novos IDs
        ativarListenerHibrido();
    } catch (e) {
...
async function salvarObra(obraData) {
    const obraParaSalvar = {
        ...
        dataCriacao: obraData.dataCriacao || firebase.firestore.FieldValue.serverTimestamp(),
    ...
    try {
        ...
        // Salvar fotos na subcoleção (se houver fotos novas)
        if (obraData.fotos && obraData.fotos.length > 0) {
            const fotosRef = obraRef.collection('fotos');
            for (const base64 of obraData.fotos) {
                await fotosRef.add({ base64, dataCriacao: firebase.firestore.FieldValue.serverTimestamp() });
            }
        }

        // Registrar histórico na subcoleção
...
function getObrasFiltradas() {
    ...
    let ativas = obras.filter(o => {
        if (!verArq && o.status === 'fechada' && o.dataAtualizacao && (agora - o.dataAtualizacao.toDate()) > 30 * 24 * 3600 * 1000) return false;
    ...
    switch (ord) {
        case 'recentes': ativas.sort((a, b) => (b.dataCriacao?.toDate() || 0) - (a.dataCriacao?.toDate() || 0)); break;
        case 'antigas':  ativas.sort((a, b) => (a.dataCriacao?.toDate() || 0) - (b.dataCriacao?.toDate() || 0)); break;
...
function listarObras() {
    ...
    container.innerHTML = obrasF.map(o => {
        ...
        const isNova = o.dataCriacao ? (new Date() - o.dataCriacao.toDate()) < 86400000 : false;
    ...
    // Adiciona botão "Carregar mais" se houver mais obras
...
function copiarEndereco(endereco) {
    navigator.clipboard.writeText(endereco).then(() => showToast('Endereço copiado!', 'success'));
}
...
async function replicarObra(firestoreId) {
    ...
    const novaObra = {
        ...
        estagio: original.estagio,
        status: 'pendente',
        clientePotencial: original.clientePotencial,
        coords: original.coords,
        dataProximaVisita: original.dataProximaVisita,
        unidade: usuarioLogado.unidade,
        criadoPorNome: usuarioLogado.nome,
        criadoPorNumero: usuarioLogado.numero,
        criadoPor: `${usuarioLogado.numero} - ${usuarioLogado.nome}`,
    ...
    try {
        await db.collection('obras').add(novaObra);
        showToast('Obra replicada!', 'success');
    } catch (e) {
        showToast('Erro ao replicar obra.', 'error');
    }
}

// ==================== DETALHES DA OBRA (COM SUBCOLEÇÕES) ====================
async function verDetalhe(firestoreId) {
    ...
    try {
        ...
        histSnapshot.forEach(doc => {
            const h = doc.data();
            const data = h.data ? h.data.toDate().toLocaleString('pt-BR') : '';
