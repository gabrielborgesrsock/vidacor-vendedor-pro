...
async function salvarObra(obraData) {
    const now = Date.now(); // timestamp numérico, compatível com todas as versões
    ...
    try {
        ...
        playSuccessSound();
        return obraRef.id; // retorna o ID para quem chamou
    } catch (e) {
...
function getObrasFiltradas() {
    ...
    let ativas = obras.filter(o => {
        // Se não for para ver arquivadas, filtra fechadas recentes
