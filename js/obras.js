...
function atualizarFiltrosVendedor() {
    ...
    if (mapFiltroUnidade) mapFiltroUnidade.innerHTML = htmlUnidades;
}
...
function getObrasFiltradas() {
    ...
    switch (ord) {
        case 'recentes': ativas.sort((a, b) => new Date(b.dataCriacao || 0) - new Date(a.dataCriacao || 0)); break;
        case 'antigas':  ativas.sort((a, b) => new Date(a.dataCriacao || 0) - new Date(b.dataCriacao || 0)); break;
...
function listarObras() {
    ...
    container.innerHTML = obrasF.map(o => {
        ...
        const isNova = (new Date() - new Date(o.dataCriacao || 0)) < 86400000;
...
async function verDetalhe(firestoreId) {
    ...
    try {
        ...
        histSnapshot.forEach(doc => {
            const h = doc.data();
            const data = h.data || '';
            historicoHtml += `<div class="small mb-1">${new Date(data).toLocaleString('pt-BR')}: ${h.descricao}</div>`;
        });
